...
def _xml_element_to_dict(element):
    """Recursively convert an XML element tree to a Python dict."""
    ...
    for child in children:
        ...
        if child.tag in data:
            # Convert to list when multiple child nodes have the same tag
...
def _dict_to_xml(tag, value):
    """Serialize a Python object (dict/list/primitive) into XML."""
    if isinstance(value, list):
        inner = ''.join(_dict_to_xml('item', item) for item in value)
...
def _api_response(request, data, status=200, root='response'):
    fmt = _get_response_format(request)
    if fmt == 'xml':
        xml_body = f'<?xml version="1.0" encoding="UTF-8"?>{_dict_to_xml(root, data)}'
        return HttpResponse(xml_body, status=status, content_type='application/xml')
    safe = isinstance(data, dict)
    return JsonResponse(data, status=status, safe=safe)


def _parse_payload(request):
    """Parse JSON or XML request body into a Python dict."""
    if not request.body:
        return {}, None
    ...
    if 'json' in content_type:
        try:
            ...
            return None, {'detail': 'Invalid JSON payload.'}
    elif 'xml' in content_type:
        try:
            root = ET.fromstring(request.body)
            parsed = _xml_element_to_dict(root)
        except ET.ParseError:
            return None, {'detail': 'Invalid XML payload.'}
    else:
        return None, {'detail': 'Unsupported Content-Type. Use application/json or application/xml.'}

    if isinstance(parsed, dict) and len(parsed) == 1 and 'product' in parsed:
        parsed = parsed['product']
    if not isinstance(parsed, dict):
        return None, {'detail': 'Payload must be an object/dictionary.'}
    return parsed, None


def _validate_product_payload(data, partial=False):
    """Validate and coerce incoming product data."""
    errors = {}
    validated = {}
    ...
    for field in required_fields:
        if field not in data or str(data.get(field) or '').strip() == '':
    ...
    if 'price' in data:
        try:
            validated['price'] = Decimal(str(data.get('price')))
        except (InvalidOperation, TypeError):
            errors['price'] = 'Price must be a valid decimal number.'

    if 'ingredients' in data:
        validated['ingredients'] = str(data.get('ingredients') or '')

    if 'stock' in data:
        try:
            stock_val = int(data.get('stock'))
            if stock_val < 0:
                errors['stock'] = 'Stock must be zero or positive.'
            else:
                validated['stock'] = stock_val
        except (TypeError, ValueError):
            errors['stock'] = 'Stock must be an integer.'

    if 'is_active' in data:
        bool_val = _coerce_bool(data.get('is_active'))
        if bool_val is None:
            errors['is_active'] = 'Use true/false for is_active.'
        else:
            validated['is_active'] = bool_val

    if 'is_archived' in data:
        bool_val = _coerce_bool(data.get('is_archived'))
        if bool_val is None:
            errors['is_archived'] = 'Use true/false for is_archived.'
        else:
            validated['is_archived'] = bool_val

    if 'expiration_date' in data:
        exp_val = data.get('expiration_date')
        if exp_val in (None, ''):
            validated['expiration_date'] = None
        else:
            try:
                validated['expiration_date'] = date.fromisoformat(str(exp_val))
            except ValueError:
                errors['expiration_date'] = 'Use ISO date format YYYY-MM-DD.'
...
def api_products(request):
    """List or create products. Supports JSON and XML."""
    if request.method == 'GET':
        products = Product.objects.filter(is_archived=False).order_by('id')
        data = {'products': [_product_to_dict(p) for p in products]}
        return _api_response(request, data)

    if request.method == 'POST':
        payload, parse_error = _parse_payload(request)
        if parse_error:
            return _api_response(request, {'errors': parse_error}, status=400)
        cleaned, errors = _validate_product_payload(payload, partial=False)
        if errors:
            return _api_response(request, {'errors': errors}, status=400)
        product = Product.objects.create(**cleaned)
        return _api_response(request, {'product': _product_to_dict(product)}, status=201)

    return _api_response(request, {'error': 'Method not allowed.'}, status=405)
...
def api_product_detail(request, pk):
    ...
    try:
        ...
        return _api_response(request, {'error': 'Product not found.'}, status=404)

    if request.method == 'GET':
        return _api_response(request, {'product': _product_to_dict(product)})

    if request.method == 'PUT':
        payload, parse_error = _parse_payload(request)
        if parse_error:
            return _api_response(request, {'errors': parse_error}, status=400)
        cleaned, errors = _validate_product_payload(payload, partial=True)
        if errors:
            return _api_response(request, {'errors': errors}, status=400)
    ...
    if request.method == 'DELETE':
        ...
        if sales_items_count > 0:
            ...
            product.save()
            msg = f'Product archived because it is referenced in {sales_items_count} sale(s).'
            return _api_response(request, {'message': msg, 'product': _product_to_dict(product)}, status=200)
        try:
            product.delete()
        except ProtectedError:
            return _api_response(request, {'error': 'Product cannot be deleted because it is in use.'}, status=409)
        return _api_response(request, {'message': 'Product deleted.'}, status=200)

    return _api_response(request, {'error': 'Method not allowed.'}, status=405)


def api_docs(request):
    """Render a lightweight HTML docs page with JSON and XML examples."""
    base_url = request.build_absolute_uri('/api/products/')
    return render(request, 'core/api_docs.html', {'base_url': base_url})
